...
public class FileUtils {
    ...
    /**
     * 子孫ファイルであるかチェックする
     * @param childFile			検索ファイル
     * @param parentFile		親ファイル
     * @return			true=子孫ファイル
     */
    ...
    public static String getRelativePath(File childFile, File parentFile) {
        ...
        try {
			...
			if (childFile.getCanonicalFile().equals(parentFile.getCanonicalFile())) return "." + File.separator;
			if (childFile.isFile()) {
			    if (isEqualsFile(childFile.getParentFile(), parentFile)) return "." + File.separator + childFile.getName();
			}
			if (!childFile.exists()) {
				return null;
			}
...
//        if (slash_path.indexOf(":") != 1) // 2013/01/22 修正 yabe
     ...
     * 配下のファイルを取得する
     * @param dir	検索ディレクトリ
     * @param exclude		除外ファイル名（ワイルドカード可）カンマ区切りで指定
     * @param excludePath	システムフォルダなどのアプリケーション固有の除外パス
     * @return     子ファイルリスト
     */
