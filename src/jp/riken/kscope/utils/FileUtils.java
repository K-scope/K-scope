...
public class FileUtils {
     ...
     * @return        モジュール名
     */
    public static String normalizeName(String name, String base_path) {
        if (base_path == null) return name;

        String normalize = name;
        try {
            java.nio.file.Path path = java.nio.file.Paths.get(name);
            normalize = path.normalize().toString();
            String relative_name = FileUtils.getRelativePath(base_path, normalize);
            if (relative_name != null && !relative_name.isEmpty()) {
                normalize = relative_name;
            }
        } catch (Exception ex) {}
