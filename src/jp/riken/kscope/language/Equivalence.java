...
public class Equivalence extends jp.riken.kscope.language.Block {
    ...
    /**
     * コピーコンストラクタ。
     * @param    dest         コピー元Equivalenceクラス
     */
    public Equivalence(Equivalence dest) {
        super.clone(dest);
        if (dest == null) return;
    ...
    /**
     * Equivalence文の文字列表現を返す
     */
    @Override
    public String toString() {
        StringBuilder buf = new StringBuilder();

        if (this.variables == null || this.variables.size() <= 0) return null;
        buf.append("Equivalence");
        buf.append(" (");

        StringBuilder mem_buf = new StringBuilder();
        for (Variable var : this.variables) {
            if (mem_buf.length() > 0) mem_buf.append(", ");
            mem_buf.append(var.getVariableString());
        }
        buf.append(mem_buf);
        buf.append(")");
