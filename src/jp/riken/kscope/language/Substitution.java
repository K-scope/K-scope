...
public class Substitution extends Block {
    ...
    public List<ProcedureUsage> getCalls() {
        Set<ProcedureUsage> calls = this.getAllFunctions();
        if (calls == null || calls.size() <= 0) return null;

        return new ArrayList<ProcedureUsage>(calls);
    }

    /**
     * 変数が左辺に使用されているかチェックする
     * @param left_varname        左辺変数名
     * @return        true=変数が左辺に使用されている
     */
    public boolean isLeftVariable(String left_varname) {
        if (left_varname == null) return false;
        if (this.leftVar == null) return false;

        if (this.leftVar.hasVariable(left_varname)) {
            return true;
        }
    ...
    /**
     * 変数が右辺に使用されているかチェックする
     * @param right_varname        右辺変数名
     * @return        true=変数が右辺に使用されている
     */
    public boolean isRightVariable(String right_varname) {
        if (right_varname == null) return false;
        if (this.rightVar == null) return false;

        if (this.rightVar.hasVariable(right_varname)) {
            return true;
        }
