...
public class ValidateLanguage implements ILanguageEntry {
    ...
    private Type searchValidType(ProgramUnit program, Type errorType) {
        ...
        for (UseState useEle : program.getUseList()) {
            ...
            if (useEle.hasOnlyMember() || useEle.hasTranslation(srcname)) {
                String rename = useEle.translationReverse(srcname);
                if (rename != null) {
                    Type rename_type = new Type(rename);
                    Module useModule = language.module(useEle.getModuleName());
                    target = searchValidType(useModule, rename_type);
                    if (target != null) {
                        return target;
                    }
                }
