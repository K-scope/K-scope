...
//            }
    ...
    public void actionPerformed(ActionEvent event) {
        ...
        ProjectModel project = this.controller.getProjectModel();
        // Status message
        String message = Message.getString("projectdeletefileaction.xml.delete.status"); // XML file deletion
        if (project.getFileType() == FILE_TYPE.FORTRANLANG) {
        	message = Message.getString("projectdeletefileaction.source.delete.status"); // Delete source file
        }
        Application.status.setMessageMain(message);

        // Select selected file (select file OR get child files of selected folder)
        SourceFile[] selectedFiles = null;
        String title = Message.getString("projectdeletefileaction.xml.confirmdialog.title"); // Delete XML file / folder
        String confirm =Message.getString("projectdeletefileaction.xml.confirmdialog.message"); // Delete the selected XML file / folder, are you sure?
        if (project.getFileType() == FILE_TYPE.XCODEML_XML) {
            ...
            title = Message.getString("projectdeletefileaction.source.confirmdialog.title"); // Delete source files / folders
            confirm =Message.getString("projectdeletefileaction.source.confirmdialog.message"); // Delete the selected source file / folder, are you sure?
        }
        if (selectedFiles == null) {
            JOptionPane.showMessageDialog(frame,
            		Message.getString("projectdeletefileaction.selectdialog.message"), // Select the file / folder you want to delete.
