...
public class XcodeMLContext {
    ...
    /**
     * IBaseStatement(ファイル名、行番号)クラスを取得する。
     *
     * @param visitable
     *            現在のXMLノードクラス
     * @return DefBaseStatement(ファイル名、行番号)クラス
     */
    public IBaseStatement getInvokeBaseStatement(IXmlNode visitable) {

        // 現在XMLノードがDefBaseStatement(ファイル名、行番号)クラスであるか判断する。
        if (visitable instanceof IBaseStatement) {
            return (IBaseStatement) visitable;
        }

        // 現在要素の親要素からファイル名、開始行番号、終了行番号を取得する。
        IXmlNode node = null;
        IBaseStatement base_node = null;
        for (Iterator<IXmlNode> it = _invokeNodeStack.iterator(); it.hasNext();) {
            node = it.next();
            if (node instanceof IBaseStatement) {
                base_node = (IBaseStatement) node;
                break;
            }
        }
    ...
    public String getStatementLabel() {
        if (_previousNode instanceof StatementLabel) {
            Name name = ((StatementLabel) _previousNode).getName();
