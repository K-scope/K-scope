    ...
    public VariableDimension parseVariableDimension(List<IXmlNode> arrayNodes) {
        ...
        for (IXmlNode elem : arrayNodes) {
            if (elem instanceof ArrayIndex) {
                ...
                boolean is_assumed_shape = XmlNodeUtil.isBoolean(((IndexRange) elem).isIsAssumedShape());
                boolean is_assumed_size = XmlNodeUtil.isBoolean(((IndexRange) elem).isIsAssumedSize());
                // 配列下限
