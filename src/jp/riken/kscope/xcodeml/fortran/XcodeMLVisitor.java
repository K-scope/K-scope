...
//      return result;
    ...
    public boolean enter(TypeTable visitable) {
        ...
        List<IXmlNode> list = visitable
                .getFbasicTypeOrFfunctionTypeOrFstructType();
        if (list != null) {
    ...
    public boolean enter(Alloc visitable) {
        ...
        if (_context.isInvokeNodeOf(FallocateStatement.class, 1)) {
            ...
            DefModelArraySubscriptSequence defModel = new DefModelArraySubscriptSequence(
                    visitable.getDefModelArraySubscript());
            IXmlNode[] arraySubscriptChoice = defModel
                    .getIndexRangeOrArrayIndex();
            if ((arraySubscriptChoice != null)
                    && (arraySubscriptChoice.length > 0)) {
    ...
    public boolean enter(FcontainsStatement visitable) {
        ...
        FfunctionDefinition[] array = list
                .toArray(new FfunctionDefinition[0]);
        if (_invokeEnter(array) == false) {
            return false;
        }
    ...
    public boolean enter(FstructConstructor visitable) {
        ...
        if (typeChoice == null) {
            _context.setLastErrorMessage(XmlNodeUtil.formatError(visitable,
                    EnumError.XCODEML_TYPE_NOT_FOUND, visitable.getType()));
            return false;
        } else if ((typeChoice instanceof FstructType) == false) {
            _context.setLastErrorMessage(XmlNodeUtil.formatError(visitable,
                    EnumError.XCODEML_TYPE_MISMATCH, "struct definition",
                    typeChoice.getClass().getSimpleName(), "FstructType"));
            return false;
        }
    ...
    public boolean enter(FstructDecl visitable) {
        ...
        if (typeChoice == null) {
            _context.setLastErrorMessage(XmlNodeUtil.formatError(visitable,
                    EnumError.XCODEML_TYPE_NOT_FOUND, nameElem.getType()));
            return false;
        } else if ((typeChoice instanceof FstructType) == false) {
            _context.setLastErrorMessage(XmlNodeUtil.formatError(visitable,
                    EnumError.XCODEML_TYPE_MISMATCH, "struct definition",
    ...
    public boolean enter(FunctionCall visitable) {
        ...
        if (StringUtils.isNullOrEmpty(functionName)) {
            ...
            _context.setLastErrorMessage(XmlNodeUtil.formatError(
                    functionNameElem, EnumError.XCODEML_SEMANTICS,
