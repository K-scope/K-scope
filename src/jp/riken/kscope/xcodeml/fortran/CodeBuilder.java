...
import jp.riken.kscope.xcodeml.fortran.xml.DefModelArraySubscriptSequence;
...
import jp.riken.kscope.xcodeml.fortran.xml.FequivalenceDeclSequence;
import jp.riken.kscope.xcodeml.fortran.xml.GotoStatementSequence;
import jp.riken.kscope.xcodeml.fortran.xml.IDefBaseStatement;
import jp.riken.kscope.xcodeml.fortran.xml.IXmlNode;
import jp.riken.kscope.xcodeml.fortran.xml.IXmlTypeTableChoice;
import jp.riken.kscope.xcodeml.fortran.xml.XmlSymbol;
import jp.riken.kscope.xcodeml.fortran.xml.gen.*;
...
//        return false;
    ...
    private XcodeMLTypeManager.TypeList getTypeList(String type) {
        ...
        try {
            ...
            _context.setLastErrorMessage(XmlNodeUtil.formatError(
                    _context.peekInvokeNode(), EnumError.XCODEML_CYCLIC_TYPE,
                    type));
            return null;
        }
        ...
        if (typeList == null || typeList.isEmpty()) {
            ...
            _context.setLastErrorMessage(XmlNodeUtil.formatError(
                    _context.peekInvokeNode(),
                    EnumError.XCODEML_TYPE_NOT_FOUND, type));
            return null;
        }
    ...
    public boolean writeIndexRangeArray(IXmlNode[] indexRangeArray) {
        ...
        for (IXmlNode arraySubscriptChoice : indexRangeArray) {
            ...
            if ((arraySubscriptChoice instanceof IndexRange)
                ...
                _context.setLastErrorMessage(XmlNodeUtil.formatError(
                        arraySubscriptChoice, EnumError.XCODEML_SEMANTICS,
    ...
    public boolean writeFunctionSymbol(XmlSymbol symbol,
        ...
        if (typeManager.isDecompilableType(funcType.getReturnType())
                && (_isUnderModuleDef() == false || XmlNodeUtil
                        .isBoolean(funcType.isIsExternal()))) {

            EnumType type = EnumType.getTypeIdFromXcodemlTypeName(funcType
                    .getReturnType());
            if (type.isPrimitive()) {
                writeToken(toCaseSensitive(type.fortranName()));
            } else {
                XcodeMLTypeManager.TypeList typeList = getTypeList(funcType
                        .getReturnType());
                if (typeList == null)
                    return false;
                ...
                if (topType instanceof FbasicType) {
                    if (writeBasicType((FbasicType) topType, typeList) == false)
                        return false;

                } else if (topType instanceof FstructType) {
                    FstructType structTypeElem = (FstructType) topType;
                    String aliasStructTypeName = typeManager
        ...
        if (_isUnderModuleDef()) {
            if (XmlNodeUtil.isBoolean(funcType.isIsPublic())
                    && isPublicEmit == false) {
                writeToken((isFirstToken ? "" : ", ") + toCaseSensitive("PUBLIC"));
                isFirstToken = false;
            } else if (XmlNodeUtil.isBoolean(funcType.isIsPrivate())
                    && isPrivateEmit == false) {
        ...
        if (isFirstToken == false) {
            ...
            if (lowType != null && (lowType instanceof FbasicType)) {
                ...
                List<IXmlNode> list = basicTypeElem.getDefModelArraySubscript();
    ...
    public boolean writeBasicType(FbasicType basicTypeElem,
        ...
        if (refTypeId.isPrimitive() == false) {
            _context.debugPrint(
                    "Top level type is basic-type, but is not primitive type. (%s)%n",
                    refName);
            if (typeList != null)
                _context.debugPrintLine(typeList.toString());
            _context.setLastErrorMessage(XmlNodeUtil.formatError(basicTypeElem,
                    EnumError.XCODEML_TYPE_MISMATCH, "top-level FbasicType",
                    refName, "primitive type"));
            return false;
        }
        ...
        if (basicTypeChoice != null) {
            if (basicTypeChoice instanceof Len) {
                if (refTypeId != EnumType.CHARACTER) {
                    ...
                    _context.setLastErrorMessage(XmlNodeUtil.formatError(
                            basicTypeChoice, EnumError.XCODEML_SEMANTICS,
    ...
    public boolean writeSymbolDecl(XmlSymbol symbol, IXmlNode visitable) {
        ...
        XcodeMLTypeManager typeManager = _context.getTypeManager();
        XcodeMLTypeManager.TypeList typeList = getTypeList(symbol
                .getDerivedName());
        ...
        if (topTypeChoice instanceof FbasicType) {
            ...
            if (writeBasicType(basicTypeElem, typeList) == false)
                return false;
            ...
            return writeFunctionSymbol(symbol, (FfunctionType) topTypeChoice,
                    visitable);
        }
        ...
        if (lowTypeChoice instanceof FbasicType) {
            ...
            if (coShape != null) {
                if (!visiter.invokeEnter(coShape))
                    return false;
            }
    ...
    private XmlSymbol _makeSymbol(String symbolName, String typeName) {
        ...
        if (symbolName.equals("**") || symbolName.equals("*")
                || symbolName.equals("/") || symbolName.equals("+")
                || symbolName.equals("-") || symbolName.equals("//")) {
    ...
    public boolean enter(CoShape visitable) {
        ...
        IndexRange[] indexRange = (IndexRange[]) list
                .toArray(new IndexRange[0]);
        if (indexRange != null) {
            if (!writeCoIndexRangeArray(indexRange))
                return false;
        }
        ...
        if (StringUtils.isNullOrEmpty(typeName) == false) {
            ...
            if (bottomTypeName == null) {
                _context.setLastErrorMessage(XmlNodeUtil.formatError(visitable,
                        ...
                        XmlNodeUtil.getElementName(visitable), typeName));
                return false;
            }

            EnumType typeId = EnumType
                    .getTypeIdFromXcodemlTypeName(bottomTypeName);
    ...
    public boolean enter(FdoStatement visitable) {
        ...
        FdoStatementSequence doStatementSequence = new FdoStatementSequence(
                visitable.getVar(), visitable.getIndexRange());
        if (doStatementSequence != null
                && (doStatementSequence.getVar() != null || doStatementSequence
                        .getIndexRange() != null)) {
            writeToken(" ");
    ...
    public boolean enter(FentryDecl visitable) {
        ...
        if (typeChoice == null) {
            _context.setLastErrorMessage(XmlNodeUtil.formatError(visitable,
                    EnumError.XCODEML_TYPE_NOT_FOUND,
                    functionNameElem.getType()));
            return false;
        } else if ((typeChoice instanceof FfunctionType) == false) {
            _context.setLastErrorMessage(XmlNodeUtil.formatError(visitable,
                    EnumError.XCODEML_TYPE_MISMATCH, "function definition",
                    typeChoice.getClass().getSimpleName(), "FfunctionType"));
            return false;
        }
    ...
    public boolean enter(FintConstant visitable) {
        ...
        if (StringUtils.isNullOrEmpty(typeName) == false) {
            ...
            if (typeId != EnumType.DERIVED && typeId != EnumType.INT) {
                _context.setLastErrorMessage(XmlNodeUtil.formatError(visitable,
                        EnumError.XCODEML_TYPE_MISMATCH,
                        XmlNodeUtil.getElementName(visitable), typeName, "Fint"));
                return false;
            }
        }
...
//        writeIsolatedLine(content);
    ...
    public boolean enter(FstructConstructor visitable) {
        ...
        XcodeMLTypeManager typeManager = _context.getTypeManager();
        IXmlTypeTableChoice typeChoice = typeManager.findType(visitable
                .getType());
    ...
    public boolean enter(FunctionCall visitable) {
        ...
        if (XmlNodeUtil.isBoolean(visitable.isIsIntrinsic()) == false) {
            XcodeMLTypeManager typeManager = _context.getTypeManager();
            IXmlTypeTableChoice typeChoice = typeManager
                    .findType(functionNameElem);
            if (typeChoice == null) {
                _context.setLastErrorMessage(XmlNodeUtil.formatError(visitable,
                        EnumError.XCODEML_TYPE_NOT_FOUND,
                        functionNameElem.getType()));
    ...
    public boolean enter(GotoStatement visitable) {
        ...
        if (StringUtils.isNullOrEmpty(labelName) == false) {
            ...
            GotoStatementSequence seq = new GotoStatementSequence(
                    visitable.getParams(), visitable.getValue());
    ...
    public boolean enter(IndexRange visitable) {
        ...
        if (XmlNodeUtil.isBoolean(visitable.isIsAssumedShape())
            ...
            // semantics error.
            _context.debugPrintLine("'is_assumed_shape' and 'is_assumed_size' are logically exclusize attributes.");
            _context.setLastErrorMessage(XmlNodeUtil.formatError(visitable,
                    EnumError.XCODEML_SEMANTICS,
                    XmlNodeUtil.getElementName(visitable)));
            return false;
    ...
    public boolean enter(LogNotExpr visitable) {
        // DONE: LogNotExpr
        if (writeUnaryExpr(XmlNodeUtil.getXmlNodeChoice(visitable), toCaseSensitive(".NOT."),
                true) == false) {
            return false;
        }
        ...
        if (_context.isInvokeAncestorNodeOf(FstructDecl.class) == false) {
            ...
            for (Id idElem : visitable.getId()) {
                ...
                if (typeName == null) {
                    ...
                    if (typeName == null) {
                        _context.setLastErrorMessage(XmlNodeUtil.formatError(
                                idElem, EnumError.XCODEML_NEED_ATTR, "type",
                                XmlNodeUtil.getElementName(visitable)));
                        return false;
                    }
                }
                ...
                if (symbol == null) {
                    _context.setLastErrorMessage(XmlNodeUtil.formatError(
                            idElem, EnumError.XCODEML_TYPE_NOT_FOUND, typeName));
                    return false;
                }
    ...
    public boolean enter(Value visitable) {
        ...
        if (visitable.getRepeatCount() != null) {
            ...
            if (visiter.invokeEnter(visitable.getRepeatCount()) == false)
                return false;
            writeToken("*");
        }
