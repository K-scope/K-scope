...
//        return false;
    ...
    public boolean enter(DivExpr visitable) {
        ...
        if (_context.isInvokeNodeOf(FpowerExpr.class, 1)) {
            grouping = true;
        }
        if (writeBinaryExpr(leftExpr, rightExpr, "/", grouping) == false) {
            return false;
        }
    ...
    public boolean enter(FpragmaStatement visitable) {
        ...
        if (content.startsWith("!") == false) {
            if (StringUtils.startsWithIgnoreCase(content, "omp")) {
                content = "!$" + content;
            }
            else if (StringUtils.startsWithIgnoreCase(content, "ocl")) {
                content = "!" + content;
            }
            else {
                content = "!" + content;
...
//        writeIsolatedLine(content);
    ...
    public boolean enter(IndexRange visitable) {
        ...
        if (step != null) {
            ...
            if (step.getFintConstant() != null) {
                int_value = step.getFintConstant().getValue();
            }
            // step=1の場合は省略する
            if (!("1".equals(int_value))) {
                writeToken(delim);
