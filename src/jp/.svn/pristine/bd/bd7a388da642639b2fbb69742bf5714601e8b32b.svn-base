...
public class StringUtils {
    ...
    public static String getDetectedCharset(byte[] byts) {
        ...
        for (int i = jp_char; i < len; i++) {
            ...
            if (b1 == 0x82) {
                if (b2 >= 0x40 && b2 <= 0x15) {i++; continue;}
                if (b2 >= 0x59 && b2 <= 0x5f) {i++; continue;}
                if (b2 >= 0x7a && b2 <= 0x80) {i++; continue;}
                if (b2 >= 0x9b && b2 <= 0x9e) {i++; continue;}
                if (b2 >= 0xf2 && b2 <= 0xfc) {i++; continue;}
            }
            else if (b1 == 0x83) {
                if (b2 >= 0x97 && b2 <= 0x9e) {i++; continue;}
                if (b2 >= 0xb7 && b2 <= 0xbe) {i++; continue;}
                if (b2 >= 0xd7 && b2 <= 0xfc) {i++; continue;}
            }
            else if (b1 == 0x84) {
                if (b2 >= 0x61 && b2 <= 0x6f) {i++; continue;}
                if (b2 >= 0x92 && b2 <= 0x9e) {i++; continue;}
                if (b2 >= 0xbf && b2 <= 0xfc) {i++; continue;}
            }
            else if (b1 == 0x85 || b1 == 0x86 || b1 == 0x87) {i++; continue;}
            else if (b1 == 0x88) {
                if (b2 >= 0x40 && b2 <= 0x9e) {i++; continue;}
            }
...
//                    sjis++;
     ...
     * @param target
     *            検索する文字列
     * @return キーを行番号、値を対象行先頭からの位置のリストとしたTreeMap
     *         （TreeMap<行番号, ArrayList<先頭からの位置>>）。ただし、行頭の位置を0とする。<br>
     *         textがnullあるいは空文字、またはtargetがnullあるいは空文字の場合は、空のTreeMapを
     *         返します。
     *
     *
     */
     ...
     * @return キーを行番号、値を対象行先頭からの位置のリストとしたTreeMap
     *         （TreeMap<行番号, ArrayList<先頭からの位置>>）。ただし、行頭の位置を0とする。<br>
     *         linesがnull、またはtargetがnullあるいは空文字の場合は、空のTreeMapを
     *         返します。
     *
     */
     ...
     * 2. #RRGGBB : '#'で始まり, 16進数6桁 = html色指定
     * 2. #RRGGBBAA : '#'で始まり, 16進数8桁 = 透過色
     * @param color		カラー文字列
     ...
     * @param color		java.awt.Colorオブジェクト
     * @return			HTMLカラーコード
     */
     ...
     * @return			HTML色設定文字列
     */
    public static String searchTextToHtml(String text, String search, Color forecolor, Color backcolor, int fontstyle,
     ...
     * @return			HTML色設定文字列
     */
    private static List<Integer[]> getSearchTextList(String text, String search, boolean sensitivecase, boolean regex, boolean word) {
     ...
     * @return			HTML色設定文字列
     */
    public static boolean existsSearchText(String text, String search, boolean sensitivecase, boolean regex, boolean word) {

        if (text == null) return false;
        if (search == null) return false;

        // ダミーの文字色、背景色を設定
        String html = searchTextToHtml(text, search, Color.BLACK, Color.WHITE, Font.BOLD, sensitivecase, regex, word);
     ...
     * @return			HTML色設定文字列
     */
    public static String searchWordToHtml(String text, String search, Color forecolor, Color backcolor, int fontstyle) {
     ...
     * @return				HTMLタグ適用文字列
     */
    private static String createHtmlColorTag(String text, Color forecolor, Color backcolor, int fontstyle) {
     ...
     * @return	文字列
     * @throws IOException    InputStreamエラー
     */
